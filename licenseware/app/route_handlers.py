...
class NewAppRouteHandlers:
    ...
    app_activation: RouteHandlerType = RouteHandlerType(
        url="/activate_app",
        handler=defaults.activate_app_handler
    )

    editable_tables: RouteHandlerType = RouteHandlerType(
        url="/editable_tables",
        handler=defaults.editable_tables_handler
    )

    terms_and_conditions: RouteHandlerType = RouteHandlerType(
        url="/terms_and_conditions",
        handler=defaults.terms_and_conditions_handler
    )

    refresh_registration: RouteHandlerType = RouteHandlerType(
        url="/refresh_registration",
        handler=defaults.refresh_registration_handler
    )

    app_register: RouteHandlerType = RouteHandlerType(
        url="/register_app",
        handler=defaults.register_app_handler
    )

    history_report: RouteHandlerType = RouteHandlerType(
        url="/reports/history_report",
        handler=defaults.history_report_handler
    )

    tenant_registration: RouteHandlerType = RouteHandlerType(
        url="/register_tenant",
        handler=defaults.register_tenant_handler
    )

    features: RouteHandlerType = RouteHandlerType(
        url="/features",
        handler=defaults.features_handler
    )
